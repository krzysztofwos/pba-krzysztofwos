...
void collide_particle_ball(
  ...
  const Eigen::Vector2f relative_velocity = p.velo - ball_velo;
  float impulse_magnitude = 2.f * relative_velocity.dot(plane_norm) / (1.f / particle_mass + 1.f / ball_mass);
  const Eigen::Vector2f impulse = impulse_magnitude * plane_norm;
